# GitHub Actions 완전 자동화 설계

작성: 2026-08-30 / 저장소 psyops42-art/market-brief / 최초 세팅 약 30분, 이후 무인 운영

---

## 1. 전체 구조

```
매일 22:20 UTC (= 07:20 KST) · 월~금 아침
│
├─ 1단계  fetch_data.py    핵심 시장지표 14종 수집   → data.json
│         · Yahoo Finance : 주가·환율·원자재
│         · Yahoo/FRED    : 미국채 10년/30년 (FRED 대체 경로)
│         · 한국은행 ECOS : 국고채 3년/10년
│
├─ 2단계  make_brief.py    헤드라인·코멘트 생성      → brief.json
│         · Claude API + 웹검색 도구
│         · data.json 수치를 근거로만 서술 (수치 창작 금지)
│
├─ 3단계  render.py        템플릿에 주입             → out/
│         · template.html → 2026-08-30.html
│         · make_og.py    → og-2026-08-30.png (1200×630)
│           대시보드 1·2페이지를 실제로 캡처해 폰 프레임으로 합성
│
└─ 4단계  build_index.py + git push
          · docs/ 로 복사, 아카이브 목록 재생성, 커밋·푸시
          · GitHub Pages 가 자동 배포
                    ↓
          https://psyops42-art.github.io/market-brief/2026-08-30.html
                    ↓
          카톡에 붙여넣기 → OG 썸네일 표시
```

---

## 2. 왜 이 데이터 소스인가

| 지표 | 소스 | 이유 |
|---|---|---|
| S&P500 · 나스닥100 · 코스피 | Yahoo Finance (`yfinance`) | 무료, 인증 불필요, 클라우드에서 안정적 |
| 달러인덱스 · 달러/원 · 금 · WTI | Yahoo Finance | 위와 동일 |
| 미국채 10년 · 30년 | Yahoo → FRED | 일별 관측치와 실제 날짜 사용 |
| 국고채 3년 · 10년 | 한국은행 ECOS → Npay·Reuters | ECOS 연결 장애 시 날짜가 있는 시장수익률 사용 |

국고채는 ECOS 연결을 최대 2회 시도하고, 서버에 연결되지 않으면 같은 서버의
통계코드 조합을 반복 호출하지 않습니다. 대체 경로는 Npay 공개 JSON의
`KR3YT=RR/prices`, `KR10YT=RR/prices`이며, 관측일이 수집 상한 이내인
두 일별 값으로 bp 변화를 계산합니다. ECOS 통계와 Reuters 시장수익률은
산출 기준이 다를 수 있으므로 화면에 실제 사용한 출처를 표시합니다.

---

## 3. 필요한 것

**GitHub Secrets 3개** (저장소 → Settings → Secrets and variables → Actions)

| 이름 | 발급처 | 비용 |
|---|---|---|
| `ANTHROPIC_API_KEY` | console.anthropic.com | 종량제 (아래 참조) |
| `FRED_API_KEY` | fredaccount.stlouisfed.org/apikeys | 무료 |
| `ECOS_API_KEY` | ecos.bok.or.kr/api | 무료 |

**비용 추정** — 하루 1회, 웹검색 8회 + 4천 토큰 생성 기준으로 **월 2~4달러** 수준입니다.
GitHub Actions는 Public 저장소면 무료입니다.

---

## 4. 세팅 순서

**1) 저장소 준비**
```
git clone https://github.com/psyops42-art/market-brief.git
cd market-brief
```
아래 파일을 저장소 루트에 놓습니다.
```
fetch_data.py  make_brief.py  render.py  make_og.py  build_index.py
pipeline_utils.py
template.html
.github/workflows/daily-brief.yml
```

**2) Secrets 3개 등록** (위 표)

**3) Pages 켜기**
Settings → Pages → Deploy from a branch → `main` / `/docs`
공개 주소: `https://psyops42-art.github.io/market-brief`

**4) 첫 실행**
Actions 탭 → "데일리 마켓 브리핑" → **Run workflow** (수동 실행 버튼)

3~5분 뒤 `docs/` 에 파일이 생기고 Pages가 배포됩니다.

---

## 5. 실패에 대비한 장치

**수치 미확보 시** — 해당 행을 "확인필요"로 표기하고 나머지는 정상 생성합니다.
임의 추정값을 넣지 않습니다. 미확보 목록은 Actions 요약 화면에 표시됩니다.

**자체 점검** — `make_brief.py` 가 생성 결과를 검사합니다.
- 헤드라인 3건인가
- 각 출처에 날짜(M/D)가 있는가
- MINDSET 3개, 정리문장 3줄인가
- 필수 필드가 비지 않았는가

문제가 있으면 Actions 요약에 항목별로 출력됩니다. **배포는 막지 않습니다.**
발송 전에 눈으로 확인하는 것이 전제입니다.

**실행 실패 시** — `data.json`, `brief.json`, `out/` 이 아티팩트로 7일간 보관돼
원인을 추적할 수 있습니다.

**변경 없음 처리** — 휴장일 등으로 내용이 같으면 커밋을 건너뜁니다.

**동시 실행 보호** — 데일리와 주간 워크플로는 같은 concurrency 그룹을 사용합니다.
두 작업이 모두 `docs/index.html`과 같은 브랜치에 쓰더라도 동시에 푸시하지 않습니다.

**HTML 주입 방지** — 웹검색/AI 생성 문구는 `pipeline_utils.py`에서 모두 이스케이프하고,
강조용 `<b>` 태그만 허용한 뒤 템플릿에 넣습니다.

---

## 6. 스케줄 해설

```yaml
- cron: "20 22 * * 0-4"     # UTC 일~목 = KST 월~금
```

| UTC | KST | 반영되는 마감 |
|---|---|---|
| 일 22:20 | **월 07:20** | **지난 금요일** 뉴욕·국내 마감 |
| 월 22:20 | 화 07:20 | 월요일 뉴욕·국내 마감 |
| 목 22:20 | 금 07:20 | 목요일 뉴욕·국내 마감 |

위 표는 휴장일이 없는 주의 예시입니다. 공통 `cutoff`는 수집 상한이고,
`market_dates.py`에서 미국 주식·미국채·한국·상해·유럽 시장의 캘린더를 구분합니다.
예를 들어 2026-09-08 아침에는 미국 노동절을 반영해 미국 주식·국채는 9/4,
한국·상해·유럽 주식은 9/7 종가를 기대합니다. `expected_asof`와 실제 `asof`를
비교하므로 정상 휴장은 지연 경고가 되지 않고, 열린 시장의 오래된 값은 경고합니다.

Yahoo 일별 이력이 늦으면 범위 조회로 재확인합니다. 그래도 늦을 때에는 실제
시세 날짜가 기대 거래일과 일치하고 거래소 마감 이후인 경우에만 날짜가 있는
시세로 보완합니다. 전일 등락은 직전 거래일 관측치로 계산하며, 날짜 없는
실시간 값이나 당일 장중 값으로 과거 종가를 대체하지 않습니다.

뉴스는 시장 종가와 별도로 당일 실행 시각까지 최근 72시간을 확인합니다.
당일 기사를 우선 검색하고, CNBC·BBC·연합뉴스 RSS의 날짜 있는 후보를 제공합니다.
표시 날짜는 원문 발행시각을 한국시간으로 변환해 생성합니다. 날짜가 다른 경우
`한국시간`을 표시합니다. 과거 날짜의 브리핑을 재생성할 때는 해당 날짜 07:00이 상한입니다.
최대 3회 검색하며 정상 기사는 보존합니다. 부족한 자리는 검증된 RSS 경제·금융
기사의 원문 제목과 요약으로 보충하고, 실패한 기사의 내용은 투자마인드에 남기지
않습니다. 보충 시 투자마인드는 일반 자산배분 원칙으로 작성합니다. 점검 사항은
실행 리포트에 남기며, 모든 출처가 실패하면 확인되지 않은 뉴스를 만들지 않습니다.

> GitHub Actions의 cron은 부하에 따라 **5~30분 지연될 수 있습니다.**
> 정시 발송이 중요하면 `0 21 * * 0-4`(06:00 KST)로 앞당기고 여유를 두세요.

---

## 7. 매일 하는 일

주간 화면은 `template_weekly.html`의 표 머리글과 `render_weekly.py`가 생성하는
행으로 구성합니다. 기준일 안내는 표 아래에 한 번 표시하고, '주간 최고·최저',
마켓맵(Finviz)·심리지수(CNN) 링크를 유지합니다. 템플릿 수정 시 렌더러에서
표 전체나 머리글을 중복 생성하지 않도록 두 파일의 역할을 함께 확인합니다.

주간 국고채도 ECOS 장애 시 Npay·Reuters의 일별 이력으로 대체합니다.
현재 시세를 과거 주의 금리로 사용하지 않으며, 보고 대상 금요일 이전 관측치로
주간 변화와 YTD(bp)를 계산합니다. 과거 비교값만 부족하면 확보한 금리는 표시하고
해당 변화율은 `－`로 남깁니다. 대체 시세의 출처는 각 행에 표시합니다.

**아무것도 안 해도 됩니다.** 07:20에 자동 실행을 시작하며 생성이 완료되면 배포됩니다.

다만 첫 2~3주는 아침에 링크를 열어 아래만 확인하시길 권합니다.
- 기준일 표기가 맞는가
- "확인필요"로 빠진 항목이 있는가
- 헤드라인의 출처·날짜가 붙어 있는가

내용을 고치고 싶으면 `docs/2026-08-29.html` 을 직접 수정해 커밋하면 됩니다.

---

## 8. 디자인을 바꾸고 싶을 때

`template.html` / `og_template.html` 만 고치세요. 파이썬 파일은 건드릴 필요가 없습니다.

토큰 자리에 값이 들어갑니다.

| 토큰 | 내용 |
|---|---|
| `{{TITLE}}` `{{DATE_LINE}}` | 제목·기준일 |
| `{{NEWS}}` `{{MINDSET}}` `{{QUOTES}}` | 카드 블록 |
| `{{TBL_EQUITY}}` `{{TBL_RATES}}` `{{TBL_FX}}` | 지표 표 3개 |
| `{{ONELINE_MARKET}}` `{{ONELINE_PENSION}}` `{{NEXT}}` `{{FOOTNOTE}}` | 하단 |
| `{{OG_URL}}` `{{OG_IMAGE}}` `{{OG_DESC}}` | 링크 미리보기 |

데일리 썸네일은 제목·핵심 지표 4개·오늘의 한 줄을 전체 폭으로 표시합니다. 레이아웃·글꼴·색상은 `make_og.py` 안에 있습니다.

---

## 9. 반드시 짚고 갈 점

**저장소가 Public이면 브리핑이 인터넷에 공개됩니다.** 검색엔진에도 노출될 수 있습니다.
퇴직연금 상담자료를 공개 배포하는 것이 준법감시 기준에 맞는지 먼저 확인하세요.
현재 대시보드에는 회사명이 들어가 있지 않고 하단에 원금손실 고지가 있지만,
그것만으로 대외자료 심의 요건이 충족되는지는 별개입니다.

**자동 생성물을 검토 없이 대외 공유하지 마세요.** 헤드라인은 AI가 웹검색으로 고른
것이라 맥락 오독이나 오래된 기사 선택이 있을 수 있습니다. 자체 점검 장치를 넣었지만
사람의 확인을 대체하지는 못합니다.

**API 키가 저장소 코드에 들어가지 않게 하세요.** 반드시 Secrets로만 넣습니다.
Public 저장소에 키가 커밋되면 즉시 유출됩니다.

데일리 고정 공유 주소: https://psyops42-art.github.io/market-brief/daily.html
매일 같은 파일을 최신 브리핑으로 교체합니다. 날짜별 파일은 아카이브에 보관하고,
알림·실행 요약은 고정 주소를 사용합니다. GitHub 예약 실행은 지연될 수 있습니다.
